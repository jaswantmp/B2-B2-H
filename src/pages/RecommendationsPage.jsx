...
const FIT_COLORS = {
  'AI/ML':      'bg-violet-900/40 border-violet-700/50 text-violet-300',
  'Backend':    'bg-cyan-900/40   border-cyan-700/50   text-cyan-300',
  'Design':     'bg-pink-900/40   border-pink-700/50   text-pink-300',
  'Frontend':   'bg-blue-900/40   border-blue-700/50   text-blue-300',
  'Blockchain': 'bg-amber-900/40  border-amber-700/50  text-amber-300',
  'Web3':       'bg-orange-900/40 border-orange-700/50 text-orange-300',
  'UI':         'bg-indigo-900/40 border-indigo-700/50 text-indigo-300',
}
...
function RecCard({ rec, onInvite }) {
  ...
  const score     = MATCH_SCORES[rec.id] ?? 80
  ...
  return (
    ...
    >
      ...
      {/* AI explanation */}
      <div
        className="rounded-xl p-4 mb-4 border border-violet-800/30"
        style={{ backgroundColor: 'var(--bg-raised)' }}
      >
        <div className="flex items-center gap-1.5 mb-2">
          <Sparkles size={13} className="text-violet-400 flex-shrink-0" aria-hidden="true" />
      ...
      <div className="flex gap-2 pt-3 border-t theme-divider">
        ...
        <button
          className="px-4 py-2.5 rounded-xl theme-btn-ghost text-sm font-medium transition-colors"
        >
          Profile
        </button>
      </div>
...
export default function RecommendationsPage() {
  ...
  const topRec    = recs.length ? recs.reduce((a, b) => (MATCH_SCORES[a.id] ?? 0) > (MATCH_SCORES[b.id] ?? 0) ? a : b) : null
    ...
    <div className="p-6 lg:p-8 max-w-4xl">
      ...
      <div className="mb-6">
        <div className="flex items-center gap-2.5 mb-1">
          <Sparkles size={22} className="text-violet-400" aria-hidden="true" />
      ...
      <div className="grid grid-cols-3 gap-3 mb-8">
        {[
          { icon: Users,      label: 'Recommendations', val: recs.length, sub: `${visible.length} active`, color: 'text-violet-400' },
          { icon: TrendingUp, label: 'Avg Match',        val: `${avgScore}%`,         sub: 'Above threshold',        color: 'text-emerald-400' },
          { icon: Star,       label: 'Top Match',        val: topRec ? topRec.builder.name : 'N/A',    sub: topRec ? `${MATCH_SCORES[topRec.id]}% fit` : '0% fit', color: 'text-amber-400' },
      ...
      {/* Disclaimer */}
      <div
        className="flex items-start gap-3 rounded-xl p-4 mb-6 border border-violet-800/30"
        style={{ backgroundColor: 'var(--bg-raised)' }}
      >
        <Sparkles size={15} className="text-violet-400 flex-shrink-0 mt-0.5" aria-hidden="true" />
        <p className="text-sm theme-text-secondary leading-relaxed">
          Matches are generated for <span className="text-violet-400 font-medium">HackIndia 2026</span> based
      ...
      {visible.length === 0 ? (
        <div className="text-center py-20">
          ...
          <button onClick={() => setDismissed([])} className="mt-4 text-sm text-violet-400 hover:text-violet-300 transition-colors">
            Restore dismissed
          </button>
        </div>
      ) : (
