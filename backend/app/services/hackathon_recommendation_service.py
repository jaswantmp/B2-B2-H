            ...
            for item in (hk.tracks + hk.tags):
                ...
                for sk in KNOWN_TECHNICAL_SKILLS:
                    if sk == norm_item or sk in norm_item or norm_item in sk:
                        matched_skill = sk
                        break
                if matched_skill and matched_skill not in user_skills:
                    if item not in display_missing_skills:
                        display_missing_skills.append(item)

            # G. Recommendation Match Label
            if total_score >= 90:
